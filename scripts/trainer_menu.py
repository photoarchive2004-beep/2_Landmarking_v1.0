...
# === GM Landmarking: show_model_settings (auto-generated) ===
def show_model_settings(landmark_root):
    """
    Print current HRNet training settings from config/hrnet_config.yaml
    with simple English explanations. The landmark_root argument is kept
    for compatibility but is not used.
    """
    from pathlib import Path
    import yaml
