...
def load_base_localities(tool_dir: Path) -> Path | None:
    ...
    if not last_base.exists():
        print("[ERR] cfg/last_base.txt not found; run 1_ANNOTATOR.bat or 2_TRAIN-INFER_HRNet.bat first.", file=sys.stderr)
        return None
...
def scan_localities(base_dir: Path):
    ...
    for loc_dir in sorted(p for p in base_dir.iterdir() if p.is_dir()):
        ...
        if n_images == 0:
            continue
        # считаем только ландмарки, scale-файлы не считаем
        csv_files = [p for p in png_dir.glob("*.csv") if not p.name.endswith(".scale.csv")]
        n_labeled = len(csv_files)
