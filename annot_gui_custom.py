    ...
    try:
        ...
        for line in [l.strip() for l in txt.splitlines() if l.strip()]:
            ...
            if len(parts)==2*N:
                ...
                pts=[(vals[i],vals[i+1]) for i in range(0,2*N,2)]
                best=pts; break
        return best
    except: return []
...
def move_to_bad(img_path):
    ...
    # move image and companion CSV if exists
...
class AnnotGUI(tk.Tk):
    def __init__(self, root, png_dir, n_points, csv_format="tpsutils", start_from=None):
        ...
        self.idx = 0
        if start_from:
            # find by basename (without extension ok)
        ...
        self.radius = 6
        self.zoom = 1.0
        self.offset = [0.0, 0.0]   # pan (display px)
        self.dragging_pt = None    # index of point
        self.dragging_canvas = False
        self.ctrl_down = False
        self.space_down = False

        # data
        self.pts = []              # current image pts in IMAGE coords
        self.photo = None          # PhotoImage
        self.img = None            # PIL.Image
        self.drawn = []            # canvas ids
    ...
    def save_current(self):
        ...
        if len(self.pts)>0:
            ...
            # empty: remove csv if exists
            try:
                if csv_path.exists(): csv_path.unlink()
            except: pass

    def auto_save_on_switch(self):
        # if mismatch and >1 -> ask with default Yes
    ...
    def redraw(self):
        ...
        # draw image
        w,h = self.img.size
        disp = self.img.resize((max(1,int(w*self.zoom)), max(1,int(h*self.zoom))), Image.NEAREST)
    ...
    # closest point index (display coords)
    ...
    def on_left_click(self, e):
        # if near existing point -> start drag; else add new if capacity not exceeded
    ...
    def on_left_drag(self, e):
        if self.dragging_pt is not None:
            ix,iy = self.d2i(e.x, e.y)
            self.pts[self.dragging_pt]=(ix,iy)
            self.redraw()
    ...
    def on_wheel(self, e):
        ...
        if (e.state & 0x0004)==0:  # Control mask
            return
    ...
    def on_key(self, e):
        k=e.keysym.lower()
        if k=="plus" or k=="equal":   # '+' on many keyboards
            self.radius=min(30,self.radius+1); self.redraw()
        elif k=="minus":
            self.radius=max(2,self.radius-1); self.redraw()
        elif k=="right":
            self.next_image()
        elif k=="left":
            self.prev_image()
            ...
            if j is not None:
                self.pts.pop(j); self.redraw()
        elif k=="b" and (e.state & 0x0004):  # Ctrl+B -> move image to bad
            if messagebox.askyesno("Move to bad","Move this image (and CSV) to 'bad' folder?", default="yes"):
                move_to_bad(self.img_path)
                self.after(10, self.next_image)
        elif k=="h":
            messagebox.showinfo("Help",
                "Left/Right: prev/next\n"
                "Mouse L: add/drag point\n"
                "Mouse R drag: pan\n"
                "Ctrl + wheel: zoom\n"
                "+/- : point size\n"
                "Delete: remove point under cursor\n"
                "Ctrl+B: move image to bad\n"
    ...
    # search (Ctrl+F / F): enter partial name (no extension)
